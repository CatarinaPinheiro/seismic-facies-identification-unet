...
def input_crop(data, labels, cropsize, stride):
    ...
    for depth in range(0, data.shape[0]):
        ...
        n_crops_x = int(
            ((img.shape[0] - cropsize[0]) / stride[1]) + 1)  # similar to feature map calculation with zero padding
        n_crops_y = int(((img.shape[1] - cropsize[1]) / stride[1]) + 1)
        for i in range(0, n_crops_x):
            for j in range(0, n_crops_y):
                img_patches.append(
                    img[i * stride[0]: cropsize[0] + (i * stride[0]), j * stride[1]: cropsize[1] + (j * stride[1])])

                if i == n_crops_x - 1 and cropsize[0] + (i * stride[0]) < img.shape[0]:
                    img_patches.append(
                        img[img.shape[0] - cropsize[0]: img.shape[0], j * stride[1]: cropsize[1] + (j * stride[1])])
                if j == n_crops_y - 1 and cropsize[1] + (j * stride[1]) < img.shape[1]:
                    img_patches.append(
                        img[i * stride[0]: cropsize[0] + (i * stride[0]), img.shape[1] - cropsize[1]: img.shape[1]])

                label_patches.append(
                    label[i * stride[0]: cropsize[0] + (i * stride[0]), j * stride[1]: cropsize[1] + (j * stride[1])])

                if i == n_crops_x - 1 and cropsize[0] + (i * stride[0]) < label.shape[0]:
                    label_patches.append(label[label.shape[0] - cropsize[0]: label.shape[0],
                                         j * stride[1]: cropsize[1] + (j * stride[1])])

                if j == n_crops_y - 1 and cropsize[1] + (j * stride[1]) < label.shape[1]:
                    label_patches.append(label[i * stride[0]: cropsize[0] + (i * stride[0]),
                                         label.shape[1] - cropsize[1]: label.shape[1]])
...
def convert_1_to_3_channel(input_data):
    rgb_input = []
    for i in range(0, len(input_data)):
        rgb_input.append(skimage.color.gray2rgb(input_data[i]))
